...
| `diag-city-spelling-dupes.ts` | Does cross-source dedup survive **Bangalore** vs **Bengaluru**? `clusterKey` is a normalized title + IST day, so if normalization did not fold the two spellings the same event listed on Luma and Meetup would render as two cards. **Suspected bug, DISPROVED — keep it that way.** Re-measured 2026-08-24: both spellings produce the identical key (`founders running club\|2026-08-30`), and 0 of 1237 upcoming events form a spelling-caused duplicate group. Re-run after touching the `clusterKey` normalizer; a non-zero count is a normalizer regression, not a cleanup job. Read-only. |
