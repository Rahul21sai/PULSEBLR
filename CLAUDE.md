...
| `diag-event-visibility.ts` | Does the daily digest leak another user's **private** events? Seeds a private and a pending event owned by user A, then asserts user B's digest excludes both while A's includes them, and that neither venue marker appears anywhere in B's payload. **Both directions matter** — a guard that returns nothing passes every leak test and empties the digest for everyone. Write-then-delete fixture in a `finally`; needs a DB, no server, no sign-in. Exits non-zero on regression. |
...
| `diag-design-tokens.ts` | **Is the design system in effect in a BROWSER, or only in the source?** Reads the resolved `font-family`, the nine palette values, the seven `.ty-*` steps, the spacing tokens and the structure primitives out of a real Chromium, then judges them against the specification. Exists because every failure here is silent: a literal family name in `--font-sans` falls through to `system-ui` with no error (this shipped), a `.ty-*` rule that did not compile still renders readable text, and an undefined custom property makes its whole declaration invalid at computed-value time so `padding: var(--s-4)` becomes `0px`. **Two controls that must fire** — a paper-on-white contrast pair, because a probe here once returned exactly `1.00` for 22 categories from a parser bug reading as a clean result, and an undefined `.ty-` class, which must not coincidentally match a step. Also asserts the SEMANTIC face split (serif = the city's content, sans = the product's voice) rather than merely that both faces loaded. Read-only; needs a dev server, no sign-in, no DB. `PB_BASE=... npx tsx scripts/diag-design-tokens.ts [/route ...]`. Exits non-zero on any contradiction. |
...
> **SUPERSEDED 2026-09-12 — `.meter` NOW RENDERS NOWHERE. Read the paragraph below as the reasoning
> that justified removing it, not as current behaviour.** The UI direction is explicit: *"Never
> render `connectionScore` as a number, bar, meter, star rating, or percentage. It may only express
> itself as row order, plus at most one factual clause derived from the event's own fields."* The
> argument below — that three bars are honest where "83" is not — is right as far as it goes, and
> the direction takes it one step further: **three bars assert three distinguishable levels**, which
> is the same claim to resolution drawn instead of written. `ConnectionMeter` and `cardReasonLine`
> are deleted; `.meter` survives in `globals.css` with no consumer. What persuades a reader now is
> `score-reason.ts`'s measured clauses, which come from calling the real scorer and differencing —
> and ROW ORDER, which is what the ranking was always for.
>
...
### 17. What landed 2026-09-12 — the UI direction, in three phases

A design direction applied to the whole app: one committed typographic system, a nine-value palette,
hairlines instead of shadows. Phase 1 built the substrate, Phase 2 finished `/events/[id]` as the
reference surface, Phase 3 propagated it across three agent streams on disjoint file sets. The full
record — including every option **rejected** and why — is `docs/design-decisions.md`; the reference
surface's plan and self-critique are `docs/phase2-event-page.md`.

| Phase | Where | The one thing to know |
| --- | --- | --- |
| 1 — substrate | `lib/cn.ts`, `app/globals.css`, `app/components/ui.tsx` | Nine values, two faces, one merge helper. ~1,616 hex sites across 66 files. |
| 2 — reference | `app/events/[id]/**` | The primary action is a **sticky bar**; facts now precede the verdict. |
| 3 — propagation | `app/page.tsx`, `app/people/**`, `app/admin/**` + 60 more | The feed became a **day-grouped editorial list**; `EventPills.tsx` deleted outright. |

> **`app/globals.css` WAS UNLAYERED, WHICH SILENTLY BROKE EVERY UTILITY IN THE APP — and it broke the
> premise of `lib/cn.ts` at the same time.** An unlayered rule beats a layered one regardless of
> specificity or source order, and Tailwind's utilities live in `@layer utilities`. So every custom
> class here outranked every utility. Measured on real elements, before the fix:
>
> ```
...
> <h2 class="font-sans">         ->  Newsreader     the element default is unoverridable
> ```
>
> `cn()` exists so a caller's class REACHES a primitive, and its docblock states the contract as "BASE
> FIRST, CALLER LAST — a caller must be able to override". The class arrived and then lost. The helper
> was necessary and **not sufficient**, and nothing anywhere said so.
>
> Live damage on three surfaces that all read correctly in source: the feed's row clock grey and
> unbolded, "Happening now" in `--ink-2` instead of `--live`, and `/events/[id]`'s category links grey
> where the code asks for `--accent`.
>
> The `.ty-*` scale is now in `@layer components` and `h1, h2, h3` in `@layer base`. **Scoped
> deliberately** — wrapping all ~1,400 lines would re-rank ~130 rules at once, and any rule that beats
> a utility ON PURPOSE would start losing with nothing to announce it. `diag-design-tokens.ts` asserts
> composability with four override cases plus a control; the control is the important half, because
> with no utility present the step must still apply or the "fix" has broken the scale instead.

> **EVERY PROBE FAILURE IN THIS PASS WAS A CHECK THAT SILENTLY COULD NOT FAIL. None was a false
> alarm.** Six of them, and the pattern is worth more than any one:
> a contrast control reading a **live** `CSSStyleDeclaration` *after* detaching its element (every
> property `''`, so `parseFloat` gave `NaN` and it asserted nothing) · `console.log` ignoring `%-6s`
> width specifiers, so labels printed literally and every value shifted a column · a route filter using
> `startsWith('/')` when **MSYS rewrites `/tracker` to `C:/Program Files/Git/tracker`**, so the
> diagnostic checked only the home page while reporting a clean pass for surfaces it never opened · a
> `subprocess` hex classifier returning 8 where bash measured 151 · **a sticky action bar that measured
> "on screen" while the bottom nav stole its taps** — `document.elementFromPoint()` at the bar's own
> Register centre returned a NAV LINK, and only a screenshot found it, because occlusion is not a
> property of a bounding box · and an occlusion probe asking for a point BELOW THE FOLD, where
> `elementFromPoint` returns `null` — indistinguishable from "nothing is covering it".
>
> **The rule: when a measurement disagrees with an earlier one, the instrument is the first suspect,
> not the tree.** That held for a 500 on the dev server (a cached Turbopack failure, not broken CSS)
> and for 25 simultaneous diag failures (a page that had rendered nothing).

> **A TAILWIND PALETTE COLOUR IS OUTSIDE A NINE-VALUE PALETTE AS SURELY AS A HEX, AND NO `grep '#'`
> CAN SEE IT.** The token sweep reported raw hex of the palette values at 0 and was accurate as stated
> and incomplete as a claim: **73 `bg-red-50` / `hover:bg-blue-600` / `text-amber-900` utilities**
> remained across `app/`, plus `bg-black`, `border-black/5`, and an `rgba(0,0,0,0.05)` inside an inline
> `boxShadow` that no class-based sweep could reach either. Sweep both vocabularies, and check inline
> styles.

> **`--r-touch` (4px) ON TOUCHABLES, `--r-flat` (0) ON CONTAINERS — roundness now means "you can touch
> this".** Two Phase 3 agents independently hit the same blocker and neither forked a local button to
> work around it: `ui.tsx`'s `Button`/`ButtonLink`/`Chip` were `rounded-full`, so every panel row held
> two button shapes. That restraint was correct — the fix belonged in the shared primitive, and it
> landed there. `.pill` remains `9999px` for NON-interactive status pills, which is a deliberate split.

> **A COUNT IS SANS, NOT `--font-display`.** `ui.tsx`'s `Stat` set its value in `--font-display`, which
> after the rewiring resolves to **Newsreader** — so every count in the app rendered as *the city's
> content* when a count is nothing but the app speaking. The semantic rule inverted on the one element
> that is purely a number. Same trap anywhere `--font-display` is used for numerals.

> **`themeColor` IN `app/layout.tsx` IS A LITERAL AND CANNOT BE A TOKEN.** It is serialised into a
> `<meta name="theme-color">`, where a `var()` resolves to nothing — so it survived all three sweep
> rounds still holding the retired cool `#F5F5F7`, framing an installed PWA's chrome in cool grey
> around a warm page. It is the most VISIBLE colour in the app and the only one no stylesheet governs.
> Keep it in step with `--paper` by hand.

> **THE PAST STATE IS DECIDED ON THE SERVER — `hasEnded()` in `lib/format.ts`, never in a component.**
> Two reasons, and the second is the real one: `Date.now()` in a render body is impure and React's own
> lint rule refuses it; more importantly `EventActions` is HYDRATED, so the server would render with
> one clock and the browser re-render with another, and an event ending within seconds of the request
> would produce a hydration mismatch — the server saying "Register" while the client says "This event
> has ended". Note `hasEnded` grants **no grace period** where `isHappeningNow` grants three hours:
> being wrong there mislabels a badge, being wrong here offers a ticket to something that has finished.

> **A STICKY BAR MUST BE POSITIONED FROM `--bottomnav-h`, NOT FROM A MEASURED CONSTANT.**
> `MobileBottomNav` is `fixed bottom-0 z-50` and `md:hidden`, so anything else at `bottom-0` is
> invisible below 768px. The nav measures 62px in headless Chromium — and that number *contains*
> `env(safe-area-inset-bottom)`, which is 0 there and ~34px on a notched iPhone. A literal 62px would
> therefore be correct exactly where nobody is looking and 34px wrong on the device the page is
> designed for. `--bottomnav-h` restates the nav's own padding formula; both sites carry a comment
> pointing at the other, because a second definition that must agree is one that will drift.

> **THREE OF THE PHASE 3 BRIEFS WERE STALE, ONE SELF-BLOCKINGLY.** `AdminDashboard` had imported
> `Card`/`Stat`/`Banner`/`EmptyState` from `ui.tsx` since `54a80fb`, so there was nothing to
> de-duplicate; `/people`'s local `FilterRail` had already been extracted into `FacetRail.tsx` — **and
> that file was in the same brief's own do-not-touch list**, making the instruction impossible as
> written; and two agents were told to sweep palette colours from files that had none. Each checked the
> code and said so rather than inventing work. This file already records the pattern twice (§14, §15);
> it is now five times. **Verify against the code and a query, never against the plan — including when
> the plan is mine.**

**Verified as a whole:** `tsc --noEmit` exit 0 run bare, `npm run lint` 0 errors, **1688 tests across
48 suites**, production build clean at **88/88 static pages**, `npm audit` 0 vulnerabilities,
`diag-design-tokens.ts` green on `/`, `/tracker` and `/calendar`. Zero horizontal overflow at 390, 768
and 1280 on every redesigned surface. A stray `0x01` byte in `app/scan/page.tsx` (present in HEAD, a
line break mangled after an em-dash) was repaired; the tree is clean of `0x00`, `0x01` and `0x08`.

**Not done, and each stated rather than smoothed:** `app/components/Sheet.tsx` and `app/digest` still
carry rounded, shadowed surfaces (outside every agent's file set), so two surfaces disagree with the
system · `FacetRail`'s chips are still `rounded-full` beside 4px inputs · eight controls on `/people`
paint under 44px, all `ui.tsx` buttons at `sm`/`md`, where the required gap is per-layout arithmetic no
shared primitive can do for its callers · **the admin console was never seen** (`/admin` redirects a
non-admin, and signing in as an allowlisted address would upsert a `User` row) so its screenshots show
the signed-out path · **the tracker's 7-column board never rendered** because the reachable account has
zero entries, leaving its documented 2126px `scrollWidth` unverified · `agenda`/`speakers` markup is
still unverified at **0 of 277** · the 768 two-column facts/verdict band from the Phase 2 plan was not
