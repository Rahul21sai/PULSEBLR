...
### 2. "Best for connections" did not visibly rank anything — FIXED AND VERIFIED
...
**Fixed in `app/page.tsx`.** A `chronological = sort === 'soonest'` flag now gates the day grouping; any ranked sort renders one flat list in the API's order.

Verified in the browser after the fix — day headings gone, order matches the score exactly:

```
1  Women In Tech Mixer - Bengaluru Tech Week 2026      100
2  KONG API + AI Summit 2026                           100
3  Databricks Campus Hackathon (BMSCE Edition)          96
4  Databricks Campus Hackathon (RVCE Edition)           96
5  Agents & APIs Bengaluru Developer Meetup             96
6  Space Tech Meetup - Bengaluru                        95
7  Building AI Agents using Amazon Bedrock AgentCore    94
8  Laravel Bengaluru: Tech Week Meetup                  91
```

Regression checked: the default view still renders the time rail — `Happening now`, `Today`, `Tomorrow`, `Wed, 26 Aug`, `Thu, 27 Aug`. Grid view, `popular`, `newest` and a text query all return 200. No console errors.

> **This edit was overwritten once by a parallel session before it stuck.** If `chronological` is missing from `app/page.tsx`, it was lost again in a merge — re-apply it; it is two small hunks.

Note while fixing: `KONG API + AI Summit 2026` scores 100 and is the **Los Angeles** event from defect 1. So the off-city leak now sits at the *top* of the good sort, which raises its priority rather than lowering it.

### 2b. Invalid input to `POST /api/tracker` returns 500, not 400

Found while exercising the tracker. Posting an unrecognised `status` returns:

```
500  {"error":"Failed to create tracker entry",
      "details":"TrackerEntry validation failed: status: `interested` is not a valid enum value for path `status`."}
```

Two problems, both small: a client validation error is reported as a server fault, and the raw Mongoose message leaks the schema path. The valid values are `New | Interested | Applied | Shortlisted | Confirmed | Attended | Skipped | Rejected` — note they are **capitalised**, which is easy to get wrong from the client. Map validation failures to 400 with a generic message.
...
### 4. 20% of the default tech feed's first page is noise

Measured by reading the first 25 rows of `/api/events?techOnly=true` under the default sort and judging each. Five clear false positives:

| Row | Event | Why it should not be there |
| --- | --- | --- |
| 7 | `25% OFF: 2 Hours to Freedom: Build a Job Hunt AI Agent…` | a discount code in the title; course selling |
| 14 | `Free Gen AI & Agentic AI Demo at eMexo` | coaching-centre lead generation |
| 17 | `The Trust Advantage: Talk to Anyone, Build Rapport…` | soft skills, not engineering |
| 18 | `FounderX Silicon Valley: VIP Gathering of Global Founders` | not tech, and in-person **off-city** |
| 24 | `Silicon Valley Business Networking (Online)` | business networking |

Four of those five share one organiser, `chennai startup idea t…`, which has **20 events** in the tech feed — an off-city, low-quality publisher that defect 1 lets through.

This **confirms the "~20% false positives" figure in the product scorecard, and corrects an earlier claim of mine that precision was ~98%.** Both numbers are real and measure different things: 98% is precision across the whole corpus; 20% is precision on *the page a user actually sees*. CLAUDE.md states the rule twice — "a count is not a ranking, and the user sees the ranking" — and the corpus figure is the flattering one.

The same measurement shows **174 of 339** tech events are online against 163 offline, and `connectionScore` correctly rates online events 15–35. So the default date sort fills the first page with correctly-low-scored online events while the 88–100 in-person meetups sit further down. Defects 2 and 4 are one defect seen from two angles.

### 5. `district` leaked into a semantic field

Fixed. The source already lives in `Event.source`.

---

## Measured status against the product scorecard

From `/api/events` and `/api/events/facets`, 2026-08-24. Corpus 1,238 upcoming / 339 tech. Two dimensions are left **unscored** rather than guessed, because they cannot be measured through the API.

| Dimension | Measured now | Evidence |
| --- | --- | --- |
| Category filters clean | good | 20 categories populated in the tech feed across 3 groups with live counts; the `Gaming/XR` garbage-bin fixed — 7 mis-tags re-decided, 0 remain |
| Ranked for connections | **now working** | scores validated 88–100 for real meetups vs 15–35 online; defect 2 fixed and verified in the browser — the ranked view now renders in score order |
| Tech events | **~80% first-page precision** | 5 false positives in the first 25 (defect 4); corpus-level tech/category disagreement down 7.2% → 0.6% |
| Open source | 24 events | `Open Source` in the tech facet; FOSS United and HasGeek live as sources |
| Company events | 26 companies | Amazon 12, MongoDB 11, Microsoft 10, Bitshala 9, PyData 8, Bengaluru Tech Week 8, Google 7 |
| Every Bangalore event | 1,238 upcoming, 9 sources | District added a whole missing category — comedy, music, theatre, cultural — none of which pollutes the tech feed |
| Hardware | **10 events** | externally capped; five independent source classes probed, none publishes machine-readable Bengaluru hardware events |
| Tracking interface | **measured, works end to end** | signed in via the dev provider and drove the whole flow through the browser: create 201 → kanban move to `Confirmed` 200 and persisted → event populated → `phase6/stats` reflected `trackedEvents: 1` → delete 200 → 0 rows left. All per-user reads 200: `/api/tracker`, `/api/phase6/{stats,follow-ups,repeat-connections}`, `/api/folders`, `/api/contacts`. One defect found (2b) |
| Production-ready | **partly measured** | auth guards verified live: `/api/tracker` returns 401 anonymous, 200 signed in, and 401 again after sign-out. `npm audit` 0 vulns and 307 tests green are a peer session's figures, not mine — I could not run them |

**What is actually blocked, stated as causes rather than percentages:**

- **Hardware is a supply ceiling, not a code problem.** 24 of 1,354 events corpus-wide carry hardware vocabulary at all. IEEE vTools is retired (404/500). IEEE Bangalore has The Events Calendar installed and answering — with **one event, from 2020**. IESA 404s, SEMI 403s. IISc has no events post type and empty feeds; IIIT-B has no `wp-json`. No classifier work creates events nobody publishes. What *was* ours to fix — the floor not knowing `VLSI`, `Verilog`, `VHDL`, `RISC-V` — is fixed.
- **Tech-feed precision is now a RANKING problem, not a classification problem.** Corpus precision is high; the first page is not. Fixing defects 1 and 2 addresses it without touching the tagger.
- **Two dimensions need a working shell to measure**, which this session no longer has.
