...
describe('isAdminOnlyPath: the operator-only pages', () => {
   ...
   *
   * It used to read `expect(isAdminOnlyPath('/add-event')).toBe(true)`, with the reasoning that
   * `POST /api/events` refused non-admins so the form was a dead end. That was correct then. The
   * user-added-events work changed the route to `requireUser()`: a regular user now gets a real
   * outcome — `visibility: 'private'` for an event only they can see, or `'pending'` to offer it to
   * the shared feed — and only `visibility: 'public'` still re-checks the admin allowlist inside
   * the handler.
   *
   * So admin-gating the page would now REMOVE a feature rather than protect anything. It stays
   * PROTECTED (signed-in), which the assertion below pins, so this is a narrowing that was dropped
   * and not a gate that was lost.
   */
  it('does NOT gate /add-event — any signed-in user may add a private event or submit one', () => {
    expect(isAdminOnlyPath('/add-event')).toBe(false);
    // …but it still requires a session. Losing THIS would make it genuinely public.
    expect(isProtectedPath('/add-event')).toBe(true);
  });
